...
program merge_mesh
    // further contract 
    use  ../external/pmids/contracted_gen_mesh_all_jrnls, clear
    replace gen_mesh = "Proteins" if strpos(gen_mesh, "Proteins")>0

    use ../output/list_of_athrs, clear
    joinby id using ../external/pmids/contracted_gen_mesh_all_jrnls
...
program merge_concepts
    use ../output/list_of_athrs, clear
    joinby id using ../external/pmids/concepts_all_jrnls
    gen count = 1
    gcollapse (sum) count (mean) score , by(athr_id term)
