 ...
 for (q in 139:140) {
    ...
    au_ids <- lapply(1:N_articles, function(i) {
      if (length(works_from_pmids[[i]][["authorships"]])!=0) {
        ...
        cbind(ids, title, pub_date, cite_count, pub_type, pmid, which_athr, athr_id, athr_name, raw_affl, num_affls)
      }
    }) %>% bind_rows()
    colnames(au_ids) <- c("id","title","pub_date", "cite_count", "pub_type", "pmid","which_athr","athr_id", "athr_name", "raw_affl", "num_affls")
    au_ids <- au_ids %>%
