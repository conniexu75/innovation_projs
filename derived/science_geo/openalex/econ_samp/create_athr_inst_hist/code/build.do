...
program merge_geo
    ...
    replace new_inst = "Brown University" if strpos(inst, "Brown University") >0 
    replace new_inst_id = "I27804330" if new_inst == "Brown University"
...
program clean_panel
    syntax, time(str)
    use ../external/top4/top4, clear
    gen date = date(pub_date, "YMD")
    gen year = yofd(date)
    keep if inrange(year, 1945, 2022)
    gcontract athr_id 
    drop _freq
    merge 1:m athr_id using ${temp}/appended_pprs, keep(3) nogen
    gen year = yofd(dofq(qrtr))
    drop if mi(`time')
    replace inst_id = "I111979921" if inst_id == "I2801857525"
    if "`time'" == "year" {
    ...
    gen econ_aff = inlist(inst, "American Economic Association", "Hoover Institution", "Center for Economic and Policy Research", "National Bureau of Economic Research", "Abdul Latif Jameel Poverty Action Lab", "Institute for Fiscal Studies", "Centre for Economic Policy Research")  | inlist(inst, "IZA - Institute of Labor Economics") 
    ...
    drop if num_affls > 1 & (funder == 1 | broad_affl == 1 | hhmi_affl == 1 | econ_aff == 1 )
    drop if inst == "Howard Hughes Medical Institute"
    drop if inst_id == "I1344073410"
    drop if inst == "National Bureau of Economic Research" | inst == "IZA - Institute of Labor Economics" | inst == "Centre for Economic Policy Research" |  inst == "Federal Reserve" 
    ...
    replace msa_c_world = city + ", " + country_code if country_code != "US" & !mi(city) & !mi(country_code)
    compress, nocoalesce
    save ${output}/top_4_filled_in_panel_`time', replace
