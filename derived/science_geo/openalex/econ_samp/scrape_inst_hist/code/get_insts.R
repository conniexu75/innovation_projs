...
################################### MAIN ###################################
insts <- read_dta('/export/scratch/cxu_sci_geo/scrape_econ_inst/list_of_insts.dta')
nr <- nrow(insts)
...
for (q in 10:10) {
  ...
  write_csv(inst_chars, paste0("/export/scratch/cxu_sci_geo/scrape_econ_inst/inst_geo_chars", as.character(q), ".csv"))
}
