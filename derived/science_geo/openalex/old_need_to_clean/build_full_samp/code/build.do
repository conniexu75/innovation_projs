...
set seed 8975
set maxvar 120000
global temp "/export/scratch/cxu_sci_geo/build_full_samp"
global output "/export/scratch/cxu_sci_geo/build_full_samp/output"
...
program append
    ...
    gen year = yofd(date(pub_date, "YMD"))
    save ${temp}/pprs_split1, replace
    clear
    forval i = 5001/10966 {
        di "`i'"
        append using ${temp}/ppr`i'
    ...
    gen year = yofd(date(pub_date, "YMD"))
    save ${temp}/pprs_split2, replace

    clear
    forval i = 1/2 {
        use ${temp}/pprs_split`i', clear
        frame put if year <= 1950, into(samp1_split`i')
        frame put if year > 1950 & year <= 1980 , into(samp2_split`i')
        frame put if year > 1980 & year <= 2000 , into(samp3_split`i')
        frame put if year > 2000 , into(samp4_split`i')
    }
    clear
    fframeappend, using(samp1*) drop 
    save ${temp}/appended_pprs_0_1950, replace
    fframeappend, using(samp2*) drop 
    save ${temp}/appended_pprs_1950_1980, replace
    fframeappend, using(samp3*) drop 
    save ${temp}/appended_pprs_1980_2000, replace
    fframeappend, using(samp4*) drop 
    save ${temp}/appended_pprs_2000_2022, replace
    ...
    by id athr_id: egen num_which_athr = sum(which_athr_counter)
    gen mi_inst = mi(inst)
    bys id athr_id: egen has_nonmi_inst = min(mi_inst)
    replace has_nonmi_inst = has_nonmi_inst == 0
    drop if mi(inst) & num_which_athr > 1
...
program clean_panel
    ...
    drop if num_affls > 1 & (funder == 1 | broad_affl == 1 | hhmi_affl == 1)
    ...
    by athr_id : egen max_mode_yr = max(mode_yr) 
...
program convert_year_to_qrtr
    ...
    save ${output}/filled_in_panel_qrtr, replace
end
main
