...
#queries <- read_dta(file = '../external/samp/nature_sub_all_pmids.dta')
#pmid <- queries$pmid
#len <- length(pmid)
#n <- ceiling(len/500)
#for (i in 1:n) {
...
#      master = data.frame(pmid = pmid[start:end], date = info[1,], mesh = info[2,],
#                          journal=info[3,], affil=info[4,], athrs = info[5,], pt = info[6,], gr = info[7,])
#      file_name = paste("../output/metadata/","natsub_",start,"_", end,".csv", sep="")
#      write_csv(master, file = file_name)
#}
queries <- read_dta(file = '../external/samp/demsci_all_pmids.dta')
pmid <- queries$pmid
len <- length(pmid)
n <- ceiling(len/500)
for (i in 401:n) {
...
#  master = data.frame(pmid = pmid[start:end], pt = info[1])
      master = data.frame(pmid = pmid[start:end], date = info[1,], mesh = info[2,],
                          journal=info[3,], affil=info[4,], athrs = info[5,], pt = info[6,], gr = info[7,])
      file_name = paste("../output/metadata/","demsci_",start,"_", end,".csv", sep="")
      write_csv(master, file = file_name)
}
