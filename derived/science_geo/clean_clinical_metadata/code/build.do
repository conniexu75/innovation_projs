...
here, set
set maxvar 120000
global temp "/export/scratch/cxu_sci_geo/clean_clinical_metadata"
program main
    global samp med
    *find_already_cleaned
    *qui clean_mesh
    di "done clean_mesh"
    *qui clean_date
    di "done clean_date"
    *qui clean_journal
    di "done clean_journal"
    *qui clean_pubtype
    di "done clean_pubtype"
    *qui clean_authors
    di "done clean_authors"
    *qui clean_affls
    di "done clean_affls"
    *qui reshape_mult_affiliations
    di "done reshape_mult_affiliations"
    *qui reshape_mesh_terms
    di "done reshape_mesh_terms"
    clean_geo
    di "done clean_geo"
    qui merge_all
end

program find_already_cleaned
    use ../external/cns_med_samp/cleaned_all_cns_${samp}, clear
    merge m:1 pmid using ../external/med_pmids/${samp}_to_clean, assert(1 2 3) keep(3) nogen keepusing(pmid)
    save ${temp}/already_cleaned_${samp}, replace
    gcontract pmid
    merge 1:1 pmid using ../external/med_pmids/${samp}_to_clean, assert(2 3) keep(2) nogen
    cap drop _freq
    save ${temp}/${samp}_remaining_to_clean, replace

    use ../external/cns_med_samp/major_mesh_terms_cns_med.dta, clear
    merge m:1 pmid using ../external/med_pmids/${samp}_to_clean, assert(1 2 3) keep(3) nogen
    save ${temp}/mesh_already_cleaned_${samp}, replace
end

program clean_mesh
    use ${temp}/${samp}_remaining_to_clean, clear
	gen mesh_na = mesh == "NA"
	tab mesh_na
    drop mesh_na
	...
	egen nterms = noccur(raw_mesh), string("<MeshHeading>")
	qui sum nterms
...
program clean_journal
    use ${temp}/cleaned_date_${samp}, replace
...
program clean_authors
	...
	egen num_athrs = noccur(athrs), string("<Author ValidYN=Y>")
	qui sum num_athrs
	...
	forval i = 1/`max_authors' {
		...
		egen num_affiliations`i' = noccur(athrs`i'), string("<Affiliation>")
	}
    compress, nocoalesce
...
program clean_affls
    ...
    compress, nocoalesce
    save ${temp}/pre_reshape_${samp}, replace
end
...
program reshape_mult_affiliations
	...
	cap drop *_raw athrs* num_affiliations* journal_na gr num_athrs nterms most_affiliations to_drop pt* start raw_pt 
	sreshape long last_name first_name `vars', i(pmid journal date journal_abbr pub_type trial) j(which_athr) missing(drop all) 	
...
program reshape_mesh_terms
    ...
    compress, nocoalesce
    save ${temp}/major_mesh_terms_${samp}, replace
end
...
/*    qui {
    ...
    di "Done prepping clean_geo"
    
    qui {
        use ${temp}/pmid_author_affiliation_list_expanded_${samp}, clear
        gen country = "" 
        gen ncountries = 0
