...
program main
    ...
    foreach t in year_second  { //year_second_cns year year_cns year_first year_first_cns year_firstlast year_firstlast_cns { 
        di "SAMP: `t'"
        ...
        qui event_study, samp(`t') timeframe(10) ymax(1) ygap(0.1) delta(excluded_inst_ln_y_diff) fes(`field') fol(field)  het_analysis(1)
...
program make_movers
    ...
    restore
    bys inst_id athr_id: gen id_cnt =  _n == 1
    bys inst_id: egen tot_athrs = total(id_cnt) 
    bys inst_id: egen tot_movers = total(id_cnt &  mover == 1)
    ...
    gen large_cities = tot_athrs  >= 100 & tot_movers >=10
    drop _merge
...
program make_dest_origin
    ...
    use if analysis_cond == 1  & large_cities == 1 using ../temp/mover_temp_`samp' , clear  
    ...
    hashsort athr_id which_place year
    foreach var in avg_ln_y excluded_inst_ln_y excluded_star_inst_ln_y inst_ln_y x inst_ln_x msa_athr_cnt msa_ln_x star_inst_ln_y inst_prob_cns inst_ln_cns_y inst_cns_athr msa_noinst_athr {
...
program event_study 
    ...
    merge m:1 athr_id using ../temp/mover_xw_`samp', assert(1 2 3) keep(3) nogen
    if strpos("`samp'" , "cns") == 0 {
        keep athr_id field year msa_comb impact_cite_affl_wt msa_size which_place inst_id move_year first_pub_yr ppr_cnt cns cns_impact_cite_affl_wt which_move city_mover
    }
    if strpos("`samp'" , "cns") > 0 {
        keep athr_id field year msa_comb impact_cite_affl_wt msa_size which_place inst_id move_year first_pub_yr ppr_cnt cns which_move city_mover
    }
...
program coathr_prod
    ...
    local post_mean : di %3.2f r(mean)
        tw rcap ub lb rel if rel != -1,  lcolor(ebblue%50) msize(vsmall) || scatter b rel if se !=0 | rel == -1, msize(small) mcolor(ebblue%50) xlab(-10(1)10, labsize(vsmall)) ylab(-2(0.2)1, labsize(vsmall)) ///
          ...
          legend(on order(- "N (Movers) = `num_movers'" ///
                                                            "Pre-period mean = `pre_mean'" ///
                                                            "Post-period mean = `post_mean'") pos(1) ring(0) size(vsmall) region(fcolor(none))) xtitle("Relative Year to Move", size(vsmall)) ytitle("Log Productivity", size(vsmall))
