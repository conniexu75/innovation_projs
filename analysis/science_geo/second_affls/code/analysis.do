...
program make_movers
    syntax, samp(str)
    use athr_id field msa_comb year inst_id msa_size impact_cite_affl_wt avg_team_size sec_* if !mi(msa_comb) & !mi(inst_id) using ../external/samp/athr_panel_full_comb_`samp', clear 
    hashsort athr_id year
    gen place_count =  1 if msa_comb != msa_comb[_n-1]
    bys athr_id: egen num_moves = total(place_count)
    bys athr_id (year): gen which_place = sum(place_count)
...
program make_dest_origin
    ...
    foreach loc in inst_id msa_comb {
        preserve
        drop if move_year == year & mover == 1
        if "`loc'" == "inst_id" {
            local suf inst 
        }
        if "`loc'" == "msa_comb" {
        ...
        bys athr_id `loc' affl_num: gen `suf'_athr_id = _n == 1
        bys `loc' : egen `suf'_athr_cnt = total(`suf'_athr_id)
        if "`loc'" == "inst_id" {
            gcollapse (mean) ln_y ln_x `suf'_athr_cnt (firstnm) msa (min) affl_num (sum) `suf'_sum_ln_y, by(`loc' ${time}) 
            gcollapse (mean) `suf'_ln_y = ln_y `suf'_ln_x = ln_x `suf'_athr_cnt (firstnm) msa (sum) `suf'_sum_ln_y (min) affl_num, by(`loc')  
            *drop if affl_num == 2
        }
        if "`loc'" == "msa_comb" {
            bys `loc' inst_id: gen inst_cnt_id = _n == 1
            bys `loc' : egen inst_cnt = total(inst_cnt_id)
            gcollapse (mean) ln_y ln_x  (firstnm) msa inst_cnt `suf'_athr_cnt (sum) `suf'_sum_ln_y (min) affl_num , by(`loc' ${time}) 
            gcollapse (mean) `suf'_ln_y = ln_y `suf'_ln_x = ln_x  inst_cnt `suf'_athr_cnt (firstnm) msa (sum) `suf'_sum_ln_y (min) affl_num, by(`loc' ) 
            *drop if inst_cnt == 1
            *drop if affl_num == 2
        }
    ...
    *gen msa_wo_inst =  (tot_msa_prod - tot_inst_prod)/(msa_athr_cnt - inst_athr_cnt)
    keep if affl_num == 1
    bys msa_comb: egen msa_prod = total(ln_y)
    bys inst_id: egen inst_prod = total(ln_y)
    bys msa_comb: egen msa_cnt = count(athr_id)
    bys inst_id: egen inst_cnt1 = count(athr_id)
    gen test2 = (msa_prod - inst_prod)/(msa_athr_cnt - inst_athr_cnt)
    gen test3 = (msa_prod - inst_prod)/(msa_cnt - inst_cnt1)
    ...
    *merge m:1 msa_comb year using ../temp/msa_`samp'_collapsed, assert(1 2 3) keep(3) nogen keepusing(msa_ln_x msa_ln_y)
