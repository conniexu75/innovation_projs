...
program main
    ...
    foreach t in year_second year year_firstlast { 
        di "SAMP: `t'"
        cns_Output, samp(`t')
        ...
        event_study, samp(`t') timeframe(10) startyr(1945) endyr(1969) delta(excluded_inst_ln_y_diff) ymax(1) ygap(0.1) fes(`main_fes') fol(main)
        event_study, samp(`t') timeframe(10) startyr(1970) endyr(1994) delta(excluded_inst_ln_y_diff) ymax(1) ygap(0.1) fes(`main_fes') fol(main)
        ...
        event_study, samp(`t') timeframe(10) ymax(1) ygap(0.1) delta(excluded_inst_ln_none_diff) yvar(ln_none) fes(`main_fes') fol(main) het_analysis(0)
        ...
        combine_figures, samp(`t')
        qui matrix_to_txt, saving("../output/tables/post_mean_`t'.txt") matrix(post_mean_`t') title(<tab:post_mean_`t'>) format(%20.4f) replace
    }
    mat sum_stats = stat_year_second , stat_year, stat_year_firstlast
    qui matrix_to_txt, saving("../output/tables/sum_stats.txt") matrix(sum_stats) title(<tab:sum_stats>) format(%20.4f) replace
...
program cns_Output
    ...
    if strpos("`samp'", "cns") == 0 {
        ...
        *replace impact_cite_affl_wt = impact_cite_affl_wt + 1 if impact_cite_affl_wt == 0
...
program sum_stats
    ...
    replace impact_cite_affl_wt = 1 + impact_cite_affl_wt if impact_cite_affl_wt == 0 
    gen ln_y = ln(impact_cite_affl_wt)
    if strpos("`samp'" , "cns") == 0 {
        *gen ln_cns_y = ln(cns_impact_cite_affl_wt)
        gen ln_cns_y = cns_impact_cite_affl_wt
    }
...
program make_dest_origin
    ...
    foreach var in avg_ln_y avg_ln_nocite excluded_inst_ln_y excluded_star_inst_ln_y excluded_inst_ln_nocite excluded_inst_ln_none inst_ln_nocite inst_ln_none inst_ln_y x inst_ln_x msa_athr_cnt msa_ln_x star_inst_ln_y inst_prob_cns inst_ln_cns_y inst_cns_athr msa_noinst_athr excluded_tot tot_ls nonfed_ls_fund fed_ls_fund {
        ...
        if inlist("`var'" ,"inst_ln_y" , "star_inst_ln_y", "excluded_inst_ln_y" , "excluded_star_inst_ln_y", "inst_prob_cns", "inst_ln_cns_y", "inst_cns_athr", "excluded_inst_ln_nocite", "excluded_tot") | inlist("`var'", "fed_ls_fund", "tot_ls" , "nonfed_ls_fund","excluded_inst_ln_none") {
            ...
            tw hist `var'_diff, frac ytitle("Share of Movers", size(small)) xtitle("`type' ${`stem'_name}", size(small)) color(edkblue) xlab(-4(1)4, labsize(small)) ylab(, labsize(small)) legend(on order(- "N (Movers) = `N'" ///
                                                            "Mean = `mean'" ///
                                                            "            (`sd')") pos(1) ring(0) size(small) region(fcolo(none)))
    ...
    binscatter2 avg_ln_y_diff inst_ln_y_diff,  mcolor(gs5) lcolor(ebblue) xlab(, labsize(small)) ylab(, labsize(small)) xtitle("Destination-Origin Difference in Log Output", size(small)) ytitle("Change in Log Output after Move", size(small)) legend(on order(- "N (Movers) = `N'" ///
                                                            "Slope = `coef'") pos(5) ring(0) size(small) region(fcolor(none)))
    graph export ../output/figures/place_effect_desc_`samp'.pdf , replace
    binscatter2 avg_ln_nocite_diff inst_ln_nocite_diff,  mcolor(gs5) lcolor(ebblue) xlab(, labsize(small)) ylab(, labsize(small)) xtitle("Destination-Origin Difference in Log Output", size(small)) ytitle("Change in Log Output after Move", size(small)) legend(on order(- "N (Movers) = `N'" ///
                                                            "Slope = `coef'") pos(5) ring(0) size(small) region(fcolor(none)))
    graph export ../output/figures/place_effect_nocite_desc_`samp'.pdf , replace
    
    gen origin_loc = msa_comb if which_place  == 1
    gen origin_inst = inst if which_place  == 1
    gen dest_loc = msa_comb if which_place  == 2
    gen dest_inst = inst if which_place  == 2
    gen dest_rank = msa_rank if which_place == 2
    hashsort athr_id which_place year
    by athr_id : replace dest_inst = dest_inst[_n+1] if mi(dest_inst)
    ...
    by athr_id : replace origin_inst = origin_inst[_n-1] if mi(origin_inst)
...
program event_study 
    ...
    cap mat drop row es 
     ...
     if "`yvar'" == "ln_nocite" | "`yvar'" == "ln_none" {
    ...
    merge m:1 athr_id using ../temp/mover_xw_`samp', assert(1 2 3) keep(3) nogen
    if strpos("`samp'" , "cns") == 0 {
        keep athr_id field year msa_comb impact_cite_affl_wt msa_size which_place inst_id move_year first_pub_yr ppr_cnt cns cns_impact_cite_affl_wt which_move city_mover affl_wt cite_affl_wt impact_affl_wt avg* unique_*
    }
    if strpos("`samp'" , "cns") > 0 {
        keep athr_id field year msa_comb impact_cite_affl_wt msa_size which_place inst_id move_year first_pub_yr ppr_cnt cns which_move city_mover affl_wt cite_affl_wt impact_affl_wt  avg* unique_*
    }
    ...
    gen ln_none = ln(cite_affl_wt)
    preserve
    contract athr_id rel year move_year
    drop _freq
    tw hist rel if inrange(rel, -10, 10), freq bin(20) xtitle("Relative Year to Move")   ytitle("Number of Movers") xlab(-10(2)10)
    graph export ../output/figures/rel_yr_mvr_hist.pdf, replace
    tw hist move_year if inrange(rel, -10, 10), freq bin(75) xtitle("Move Year")   ytitle("Number of Movers") xlab(1946(5)2020)
    graph export ../output/figures/mv_yr_hist.pdf, replace
    tw hist year if inrange(rel, -10, 10), freq bin(79) xtitle("Publication Year")   ytitle("Number of Movers") xlab(1945(5)2023)
    graph export ../output/figures/pub_yr_hist.pdf, replace
    ...
    if "`delta'" == "excluded_inst_ln_none_diff" local delta_suf = "_none" 
    ...
    if "`delta'" == "excluded_inst_ln_y_diff" & "`fol'" == "main" {
        mat post_mean_`samp' = nullmat(post_mean_`samp') \ (`post_mean' , r(sd))
    }
    if inlist("`delta'" , "star_inst_ln_y_diff", "nonfed_ls_fund_diff", "fed_ls_fund_diff" , "tot_ls_diff", "excluded_tot_diff")  {
        mat post_mean_`samp' = nullmat(post_mean_`samp') \ (`post_mean', r(sd))
    }
    save ../temp/`fol'_es_coefs_`startyr'_`endyr'_`samp'`suf', replace
    if "`cond'" == "" {
        tw rcap ub lb rel if rel != -1,  lcolor(ebblue%50) msize(vsmall) || scatter b rel if se !=0 | rel == -1, msize(small) mcolor(ebblue%50) xlab(-`timeframe'(1)`timeframe', labsize(small)) ylab(-`ymax'(`ygap')`ymax', labsize(small)) ///
          ...
          legend(on order(- "N (Movers) = `num_movers'" ///
                                                            ...
                                                            "Post-period mean = `post_mean'") pos(5) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("`yvar_name'", size(small))
    ...
    *        replace ub = 1 if ub > 1
        ...
        use ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_boston`delta_suf', clear
        gen cat = "boston"
        replace rel = rel - 0.09
        append using ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_notboston`delta_suf'
        ...
        tw rcap ub lb rel if rel != -1.09 & cat == "boston",  lcolor(lavender%70) msize(vsmall) || ///
           ...
           xlab(-`timeframe'(1)`timeframe', labsize(small)) ylab(-`ymax'(`ygap')`ymax', labsize(small)) ///
              yline(0, lcolor(black) lpattern(solid)) xline(0, lcolor(gs12) lpattern(dash))  ///
              legend(on order(2 "Movers to Boston (N = `dest_boston_num_movers')" 4 "Movers Not to Boston  (N = `not_dest_boston_num_movers')")  pos(5) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("`yvar_name'", size(small))
        graph export ../output/figures/`fol'/es`startyr'_`endyr'_`samp'_citydiff`delta_suf'.pdf, replace
        
        // top vs mid vs low cities
        use ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_top_city`delta_suf', clear
        gen cat = "top_city"
        replace rel = rel - 0.27
        append using ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_mid_city`delta_suf'
        replace cat = "mid_city" if mi(cat)
        replace rel = rel - 0.09 if cat == "mid_city"
        append using ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_low_city`delta_suf'
        ...
        tw rcap ub lb rel if rel != -1.27 & cat == "top_city",  lcolor(lavender%70) msize(vsmall) || ///
           ...
           xlab(-`timeframe'(1)`timeframe', labsize(small)) ylab(-`ymax'(`ygap')`ymax', labsize(small)) ///
              yline(0, lcolor(black) lpattern(solid)) xline(0, lcolor(gs12) lpattern(dash))  ///
              legend(on order(2 "Movers to Top 5 City (N = `top_city_movers')" 4 "Movers to Top 6-10 City (N = `mid_city_movers')" 6 "Movers to Top 11-20 City (N = `low_city_movers')")  pos(5) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("`yvar_name'", size(small))
        graph export ../output/figures/`fol'/es`startyr'_`endyr'_`samp'_cityrank`delta_suf'.pdf, replace
        
        // merge l2h h2
        use ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_l2h`delta_suf', clear
        gen cat = "l2h"
        replace rel = rel - 0.09
        append using ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_h2l`delta_suf'
        ...
        tw rcap ub lb rel if rel != -1.09 & cat == "l2h",  lcolor(lavender%70) msize(vsmall) || ///
           ...
           xlab(-`timeframe'(1)`timeframe', labsize(small)) ylab(-`ymax'(`ygap')`ymax', labsize(small)) ///
              yline(0, lcolor(black) lpattern(solid)) xline(0, lcolor(gs12) lpattern(dash))  ///
              legend(on order(2 "Low to High Output Place Movers (N = `l2h_num_movers')" 4 "High to Low Output Place Movers (N = `h2l_num_movers')") pos(5) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("`yvar_name'", size(small))
        graph export ../output/figures/`fol'/es`startyr'_`endyr'_`samp'_prodchg`delta_suf'.pdf, replace
        
        // merge s2b b2s
        use ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_s2b`delta_suf', clear
        gen cat = "s2b"
        replace rel = rel - 0.09
        append using ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_b2s`delta_suf'
        ...
        tw rcap ub lb rel if rel != -1.09 & cat == "s2b",  lcolor(lavender%70) msize(vsmall) || ///
           ...
           xlab(-`timeframe'(1)`timeframe', labsize(small)) ylab(-`ymax'(`ygap')`ymax', labsize(small)) ///
              yline(0, lcolor(black) lpattern(solid)) xline(0, lcolor(gs12) lpattern(dash))  ///
              legend(on order(2 "Small to Big Place Movers (N = `s2b_num_movers')" 4 "Big to Small Place Movers (N = `b2s_num_movers')") pos(5) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("`yvar_name'", size(small))
        graph export ../output/figures/`fol'/es`startyr'_`endyr'_`samp'_sizechg`delta_suf'.pdf, replace
        
        // merge s2b2 b2s2
        use ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_s2b2`delta_suf', clear
        gen cat = "s2b2"
        replace rel = rel - 0.09
        append using ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_b2s2`delta_suf'
        ...
        tw rcap ub lb rel if rel != -1.09 & cat == "s2b2",  lcolor(lavender%70) msize(vsmall) || ///
           ...
           xlab(-`timeframe'(1)`timeframe', labsize(small)) ylab(-`ymax'(`ygap')`ymax', labsize(small)) ///
              yline(0, lcolor(black) lpattern(solid)) xline(0, lcolor(gs12) lpattern(dash))  ///
              legend(on order(2 "Small to Big Place Movers (N = `s2b2_num_movers')" 4 "Big to Small Place Movers (N = `b2s2_num_movers')") pos(5) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("`yvar_name'", size(small))
        graph export ../output/figures/`fol'/es`startyr'_`endyr'_`samp'_sizechg22`delta_suf'.pdf, replace

        // merge young old
        use ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_young`delta_suf', clear
        gen cat = "young"
        replace rel = rel - 0.09
        append using ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_old`delta_suf'
        ...
        tw rcap ub lb rel if rel != -1.09 & cat == "young" & ub != lb,  lcolor(lavender%70) msize(vsmall) || ///
           ...
           xlab(-`timeframe'(1)`timeframe', labsize(small)) ylab(-`ymax'(`ygap')`ymax', labsize(small)) ///
              yline(0, lcolor(black) lpattern(solid)) xline(0, lcolor(gs12) lpattern(dash))  ///
              legend(on order(2 "Movers Aged < 40 (N = `young_num_movers')" 4 "Movers Aged >= 40 (N = `old_num_movers')") pos(5) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("`yvar_name'", size(small))
        graph export ../output/figures/`fol'/es`startyr'_`endyr'_`samp'_age`delta_suf'.pdf, replace
        
        // merge first and later 
        use ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_mv1`delta_suf', clear
        gen cat = "first"
        replace rel = rel - 0.09
        append using ../temp/main_es_coefs_`startyr'_`endyr'_`samp'_mvafter1`delta_suf'
        ...
        tw rcap ub lb rel if rel != -1.09 & cat == "first",  lcolor(lavender%70) msize(vsmall) || ///
           ...
           xlab(-`timeframe'(1)`timeframe', labsize(small)) ylab(-`ymax'(`ygap')`ymax', labsize(small)) ///
              yline(0, lcolor(black) lpattern(solid)) xline(0, lcolor(gs12) lpattern(dash))  ///
              legend(on order(2 "First-Time Movers (N = `first_move_num_movers')" 4 "Repeat Movers (N = `later_move_num_movers')") pos(5) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("`yvar_name'", size(small))
        graph export ../output/figures/`fol'/es`startyr'_`endyr'_`samp'_which_move`delta_suf'.pdf, replace
    }
end

program combine_figures
    syntax, samp(string)
    use if analysis_cond == 1 & inrange(year, 1945,2023)  using ../temp/mover_temp_`samp' , clear  
    merge m:1 athr_id using ../temp/mover_xw_`samp', assert(1 2 3) keep(3) nogen
    merge m:1 athr_id move_year using ../temp/dest_origin_changes_`samp', keep(3) nogen
    gunique athr_id 
    local num_movers = r(unique)
    use ../temp/main_es_coefs_1945_2023_`samp'_negi, clear
    gen cat = "main"
    qui sum b if rel > 1
    local post_mean_main : di %3.2f r(mean)
    replace rel = rel - 0.09
    append using ../temp/main_es_coefs_1945_2023_`samp'_nocite
    replace cat = "nocite" if mi(cat)
    qui sum b if rel > 1 & cat == "nocite"
    local post_mean_nocite : di%3.2f r(mean)
    replace rel = rel + 0.09 if cat == "nocite"
    tw rcap ub lb rel if rel != -1.09 & cat == "main",  lcolor(lavender%70) msize(vsmall) || ///
       scatter b rel if cat == "main", mcolor(lavender%70) msize(small) || ///
       rcap ub lb rel if rel != -0.91 & cat == "nocite",  lcolor(orange%70) msize(vsmall) || ///
       scatter b rel if cat == "nocite", mcolor(orange%70) msymbol(smdiamond) msize(small) /// 
       xlab(-10(1)10, labsize(small)) ylab(-1(.1)1, labsize(small)) ///
          yline(0, lcolor(black) lpattern(solid)) xline(0, lcolor(gs12) lpattern(dash))  ///
          legend(on order(- "N (Movers) = `num_movers'" ///
                                                            "Main Specification: Post-period mean = `post_mean_main'" ///
                                                            "No Citation-Weighting: Post-period mean = `post_mean_nocite'")  pos(5) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("Log Output", size(small))
    graph export ../output/figures/main/es1945_2023_`samp'_merged.pdf, replace
...
program coathr_locs
    ...
    tw (line share_origin_inst rel if inrange(rel,-10,10), lcolor(lavender%70))  ///
        ...
        xlabel(-10(1)10, labsize(small)) ylabel(0(0.1)1, labsize(small)) ///
        xtitle("Relative Year to Move", size(small)) ytitle("Share of co-authors", size(small)) ///
        legend(on order(1 "Co-authors from Origin Institution" 2 "Co-authors from Destination Institution" 3 "First Publication Affiliated with Destination" 4 "Move") pos(11) ring(0) size(small) region(fcolor(none))) 
    ...
    tw (line share_origin_msa rel if inrange(rel,-10,10), lcolor(lavender%70))  ///
        ...
        xlabel(-10(1)10, labsize(small)) ylabel(0(0.1)1, labsize(small)) ///
        xtitle("Relative Year to Move", size(small)) ytitle("Share of co-authors", size(small)) ///
        legend(on order(1 "Co-authors from Origin City" 2 "Co-authors from Destination City" 3 "First Publication Affiliated with Destination" 4 "Move") pos(11) ring(0) size(small) region(fcolor(none))) 
    ...
    tw (line share_inst rel if inrange(rel,-10,10), lcolor(lavender%70)) ||  ///
        ...
        xlabel(-10(1)10, labsize(small)) ylabel(0(0.1)1, labsize(small)) ///
        xtitle("Relative Year to Move", size(small)) ytitle("Share of co-authors", size(small)) ///
        legend(on order(1 "Co-authors from Same Institution" 2 "Co-authors from Same City" 3 "First Publication Affiliated with Destination" 4 "Move") pos(11) ring(0) size(small) region(fcolor(none))) 
...
program coathr_prod
    ...
    local post_mean : di %3.2f r(mean)
        tw rcap ub lb rel if rel != -1,  lcolor(ebblue%50) msize(vsmall) || scatter b rel if se !=0 | rel == -1, msize(small) mcolor(ebblue%50) xlab(-10(1)10, labsize(small)) ylab(-1(0.1)1, labsize(small)) ///
          yline(0, lcolor(black) lpattern(solid)) xline(-0.5, lcolor(gs12) lpattern(dash))  ///
          legend(on order(- "N (Movers) = `num_movers'" ///
                                                            "Pre-period mean = `pre_mean'" ///
                                                            "Post-period mean = `post_mean'") pos(5) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("Log Output", size(small))
    ...
    sum b if inrange(rel, 1,10)
    local post_mean : di %3.2f r(mean)
        tw rcap ub lb rel if rel != -1,  lcolor(ebblue%50) msize(vsmall) || scatter b rel if se !=0 | rel == -1, msize(small) mcolor(ebblue%50) xlab(-10(1)10, labsize(small)) ylab(-2(0.2)1, labsize(small)) ///
          yline(0, lcolor(black) lpattern(solid)) xline(-0.5, lcolor(gs12) lpattern(dash))  ///
          legend(on order(- "N (Movers) = `num_movers'" ///
                                                            "Pre-period mean = `pre_mean'" ///
                                                            "Post-period mean = `post_mean'") pos(1) ring(0) size(small) region(fcolor(none))) xtitle("Relative Year to Move", size(small)) ytitle("Log Output", size(small))
