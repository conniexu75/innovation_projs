...
program main
    ...
    foreach t in year_firstlast year_firstlast_cns year year_cns year_last year_last_cns year_first year_first_cns {
        di "SAMP: `t'"
        qui make_movers, samp(`t')
        sum_stats, samp(`t')
        make_dest_origin, samp(`t')
        qui output_tables, samp(`t')
        qui event_study, samp(`t') timeframe(10) ymax(1) ygap(0.1) fes(`main_fes') fol(main)
        qui event_study, samp(`t') timeframe(10) ymax(1) ygap(0.1) delta(inst_sum_ln_y_diff) fes(`main_fes') fol(main)
        qui event_study, samp(`t') timeframe(10) ymax(1) ygap(0.1) delta(star_inst_ln_y_diff) fes(`main_fes') fol(main)
        qui event_study, samp(`t') timeframe(10) ymax(1) ygap(0.1) delta(star_inst_sum_ln_y_diff) fes(`main_fes') fol(main)
        qui event_study, samp(`t') timeframe(10) startyr(1995) endyr(2023) ymax(1) ygap(0.1) fes(`main_fes') fol(main)
    }
...
program make_movers
    ...
    preserve
    *keep if mover == 1 & num_moves == 2 
    keep if mover == 1 & num_moves > 1 
    keep if which_place == 3 | which_place == 2
    ...
    drop _freq
    save ../temp/second_move_`samp', replace
    ...
    drop if has_mover == 0
    *gen analysis_cond = mover == 1 & num_moves == 2  & ((mover == 0 & _merge == 1) | (mover == 1 & _merge == 3))
...
program make_dest_origin
    syntax, samp(str)
    use ../temp/mover_temp_`samp' , clear  
    drop if mover == 1 & move_year == year
    *keep if mover == 0
    gegen msa = group(msa_comb)
    gen ln_y = ln(impact_cite_affl_wt)
    gen ln_x = ln(msa_size)
    ...
    foreach loc in inst_id msa_comb {
        ...
        bys `loc' ${time} athr_id : gen `suf'_athr_cnt = _n == 1
        bys `loc' ${time} athr_id : gen `suf'_star_cnt = _n == 1 & ln_y >= p95_value
        gen `suf'_sum_ln_y = ln_y
        gen star_`suf'_sum_ln_y = stars 
        *gcollapse (mean) `suf'_ln_y = ln_y star_`suf'_ln_y = stars `suf'_ln_x = ln_x  `suf'_athr_cnt (firstnm) msa , by(`loc' ${time}) 
        gcollapse (mean) ln_y stars ln_x  (sum) `suf'_sum_ln_y star_`suf'_sum_ln_y `suf'_athr_cnt `suf'_star_cnt (firstnm) msa , by(`loc' ${time}) 
        gcollapse (mean) `suf'_ln_y = ln_y star_`suf'_ln_y = stars `suf'_ln_x = ln_x  (sum) `suf'_sum_ln_y star_`suf'_sum_ln_y `suf'_athr_cnt `suf'_star_cnt (firstnm) msa , by(`loc') 
/*        if "`samp'" != "last" & "`samp'" != "last_cns" {
    ...
    use if analysis_cond == 1 & (which_place == 2 | which_place == 3 )  using ../temp/mover_temp_`samp' , clear  
    ...
    ** new
*    keep if inrange(rel, -18, -2) | inrange(rel, 2,18)
    **
    ...
    gen year = current_year if which_place == 2
    replace year = move_year if which_place == 3
    merge m:1 inst_id using ../temp/inst_`samp'_collapsed, assert(1 2 3) keep(3) nogen
    merge m:1 msa_comb using ../temp/msa_`samp'_collapsed, assert(1 2 3) keep(3) nogen keepusing(msa_athr_cnt) 
    ...
    *merge m:1 msa_comb year using ../temp/msa_`samp'_collapsed, assert(1 2 3) keep(3) nogen keepusing(msa_ln_x msa_athr_cnt) 
    replace inst_sum_ln_y = (inst_sum_ln_y-ln_y)/(inst_athr_cnt-1)
    replace star_inst_sum_ln_y = (star_inst_sum_ln_y)/(inst_star_cnt) if stars == 0
    replace star_inst_sum_ln_y = (star_inst_sum_ln_y-ln_y)/(inst_star_cnt-1) if stars == 1

    hashsort athr_id which_place year
    save ../output/make_delta_figs_inst_`samp', replace
    hashsort athr_id which_place year
    foreach var in avg_ln_x avg_ln_y inst_ln_y inst_ln_x star_inst_ln_y msa_athr_cnt inst_sum_ln_y star_inst_sum_ln_y {
    ...
    gcontract athr_id avg_ln_y_diff inst_ln_y_diff move_year origin_loc dest_loc msa_athr_cnt_diff star_inst_ln_y_diff inst_sum_ln_y_diff star_inst_sum_ln_y_diff
    drop _freq
...
program event_study 
    syntax, samp(str) timeframe(int) fes(str) fol(str) [delta(name) startyr(int 1945) endyr(int 2023) ymax(real 1) ygap(real 0.2)] 
    ...
    foreach cond in "" "& l2h_move== 1" "& h2l_move == 1" "& b2s_move == 1" "& s2b_move == 1"  "& old == 1" "& young == 1" {
        ...
        if "`delta'" == "inst_sum_ln_y_diff" local delta_suf = "negi"
        if "`delta'" == "star_inst_sum_ln_y_diff" local delta_suf = "star2"
        if "`delta'" == "star_inst_ln_y_diff" local delta_suf = "star"

        local suf = "`suf'`delta_suf'" 
        preserve
        mat drop _all
        reghdfe ln_y `lags' `leads' lag1 treat `int_lags' int_treat `int_leads' int_lag1  if `c' , absorb(`fes') vce(cluster inst)
