...
program get_full_mover_picture
    ...
    merge m:1 athr_id using ../external/consec/consec_athrs, keep(3) nogen
    drop _freq
    ...
    rename move_year year
    merge 1:1 athr_id year using ../external/consec/consec_panel, keep(3)
    ...
    tw rcap ub lb rel if rel != -1.09 & cat == "main",  lcolor(lavender%70) msize(vsmall) || ///
       ...
       xlab(-10(1)10, labsize(vsmall)) ylab(-1(.1)1, labsize(vsmall)) ///
          ...
          legend(on order(- "N (Movers) = `num_movers'" ///
                                                            ...
                                                            "No Citation-Weighting: Post-period mean = `post_mean_nocite'")  pos(5) ring(0) size(vsmall) region(fcolor(none))) xtitle("Relative Year to Move", size(vsmall)) ytitle("Log Output", size(vsmall))
